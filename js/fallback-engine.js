...
function buildRewriteResponse(text, context) {
  const cleaned = text.replace(/^(réécris|reformule|corrige)\s*:?/i, "").trim() || text;
  return [
    venomOpening(context.venom, context.mode.id === "almost-polite"),
    "Réécriture proposée :",
    `« ${cleaned.charAt(0).toUpperCase()}${cleaned.slice(1)} »`,
    "Version plus nette : garde le sujet, supprime les détours, ajoute une demande explicite et une échéance si nécessaire. La clarté, ce vieux concept révolutionnaire.",
    finalBite(context.venom, context.mode.id === "almost-polite")
  ].join("\n\n");
}
...
function buildListResponse(text, context) {
  ...
  const sample = items.length ? items : ["premier élément flou", "deuxième machin suspect", "troisième intention molle"];
  return [
    venomOpening(context.venom, context.mode.id === "almost-polite"),
